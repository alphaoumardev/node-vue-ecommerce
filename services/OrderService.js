...
function doCheckOrderParams(params) {
	return new Promise(function(resolve,reject) {
		...
		if(!params.order_id) info.order_number = "shop-" + uniqid();
...
module.exports.updateOrder = function(orderId,params,cb) {
	if(!orderId) return cb("订单ID不能为空");
	if(isNaN(parseInt(orderId))) return cb("订单ID必须是数字");
...
module.exports.updateAddr = function(orderId,params,cb) {
	// if(!orderId) return cb("订单ID不能为空");
	// if(isNaN(parseInt(orderId))) return cb("订单ID必须是数字");
	// params["order_id"] = orderId;
	// // doCheckOrderParams(params)
	// // .then(doUpdateOrder)
	// doUpdateAddress(params)
	// .then(doGetAllOrderGoods)
	// .then(function(info){
	// 	cb(null,info.order);
	// })
